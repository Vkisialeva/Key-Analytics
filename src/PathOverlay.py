...
def extract_recovery_paths(player_positions, fps, player_side="near"):
    ...
    recovery_paths = {"left": [], "right": [], "forward": [], "backward": []}
...
def plot_recovery_paths(recovery_paths, player_side="near"):
    ...
    ax.add_patch(
        plt.Rectangle(
            (0, 0), 1, 1, fill=True, alpha=0.2, edgecolor="black", facecolor="lightblue"
        )
    )
    ...
    if player_side == "near":
        nz = plt.Rectangle(
            (0.35, 0.75), 0.30, 0.25, fill=True, alpha=0.2, color="green"
        )
    else:
        nz = plt.Rectangle(
            (0.35, 0.00), 0.30, 0.25, fill=True, alpha=0.2, color="green"
        )

    ax.add_patch(nz)

    # Colors for each direction
    colors = {"left": "red", "right": "blue", "forward": "purple", "backward": "orange"}
    ...
    for direction, paths in recovery_paths.items():
        for path in paths:
            path = np.array(path)
            ax.plot(
                path[:, 0], path[:, 1], color=colors[direction], alpha=0.7, linewidth=2
            )
